-- ============================================================================
-- ⚠️  APPLIED MIGRATION — DO NOT EDIT THIS FILE
-- ============================================================================
-- Every table below uses CREATE TABLE IF NOT EXISTS, so editing this file after
-- it has run is a silent no-op: the change never reaches any database that
-- already has these tables. That exact mistake shipped a build referencing
-- `query_pool.observed_value` on a schema that did not have the column, and it
-- only surfaced at write time — after a full paid audit had completed.
--
-- To change this schema, add a NEW migration with
-- `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, and extend the column list in
-- `20260730_reconcile_harvest_columns.sql` so the contract test stays green.
--
-- ONE EXCEPTION: making an existing statement re-runnable. That does not change
-- the end state, it only stops the script aborting when replayed — which this
-- file did, on a `COMMENT ON COLUMN` naming a column a later migration dropped.
-- `npm run test:pivot-contract` enforces re-runnability for every migration.
-- ============================================================================
...
--
-- That drop has since landed, in `20260730_closed_pool_v2.sql`. So on any
-- database that is already current, these columns no longer exist and a bare
-- `COMMENT ON COLUMN` aborts the whole script:
--
--     ERROR: 42703: column "niche_blueprint" of relation "topical_audits"
--     does not exist
--
-- The comments are documentation, not schema, so they are skipped rather than
-- allowed to fail. A migration must survive being run against a database that
-- is ahead of it.
-- ----------------------------------------------------------------------------
DO $$
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'public'
          AND table_name = 'topical_audits'
          AND column_name = 'niche_blueprint'
    ) THEN
    ...
    IF EXISTS (
        ...
        WHERE table_schema = 'public'
          AND table_name = 'topical_audits'
          AND column_name = 'projected_score'
    ) THEN
