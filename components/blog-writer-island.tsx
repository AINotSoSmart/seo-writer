                ...
                `}>
                    <AnimatePresence mode="wait">
                        ...
                        {isGenerating ? (
                            ...
                            >
                                <div className="w-10 h-10 rounded-full flex items-center justify-center mb-4 bg-stone-50">
                                    <Loader2 className="w-5 h-5 animate-spin text-stone-600" />
                                </div>
                                <h3 className="font-semibold mb-1 text-stone-900">Crafting titles...</h3>
                                <p className="text-xs text-stone-500">Analyzing SEO trends for "{keyword}"</p>
                            </motion.div>
                        ) : hasTitles ? (
                            ...
                            >
                                <div className="flex items-center justify-between mb-4 px-1">
                                    <h3 className="font-medium text-stone-900">Select a title</h3>
                                    <button onClick={onBack} className="text-xs hover:underline cursor-pointer text-stone-500">Start over</button>
                                </div>
                                ...
                                <div className="space-y-2 mb-6 max-h-[400px] overflow-y-auto pr-1">
                                    {titles.map((title, i) => (
                                        ...
                                        >
                                            ...
                                            <span className="text-sm leading-relaxed text-stone-700">{title}</span>
                                        </button>
                                    ))}
                                </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-4">
                                ...
                                <div className="mb-4">
                                    <label
                                        className="block text-xs font-medium mb-2 pl-1 text-stone-500"
                                    >
                                        Article Type
                                    </label>
                                    <div className="grid grid-cols-3 gap-2">
                                        {ARTICLE_TYPES.map((type) => (
                                            <button
                                                ...
                                                `}
                                            >
                                                <div className={articleType === type.value ? 'text-stone-900' : 'text-stone-400'}>
                                                    {ARTICLE_TYPE_ICONS[type.value]}
                                                </div>
                                                <span className={`text-[10px] text-center leading-tight font-medium ${articleType === type.value ? 'text-stone-900' : 'text-stone-600'}`}>
                                ...
                                <div className="mb-4 space-y-2">
                                    <label
                                        htmlFor="keyword"
                                        className="block text-xs font-medium mb-1 pl-1 text-stone-500"
                                    >
                                        Keyword / Topic
                                    </label>
                                    ...
                                    <p className="text-xs tracking-tight pl-1 text-stone-400">
